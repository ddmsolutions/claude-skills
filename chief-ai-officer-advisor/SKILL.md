...
> Adapted from alirezarezvani/claude-skills (c-level-advisor/skills/chief-ai-officer-advisor/SKILL.md) at commit 19392f7, MIT licence. Adapted 27 Aug 2026: home-ecosystem dependencies removed (bundled scripts, /cs: routing, agent protocol); jurisdiction and doctrine assumptions annotated.
...
**Note:** Hard-coded market facts in this skill (API pricing, GPU rental rates, regulatory dates) are snapshots — verify against current sources before using them in a decision.

This skill does **not** cover tactical AI/ML engineering: RAG implementation, agent design, prompt engineering, eval infrastructure, model deployment, or cost optimization are engineering concerns, not covered here.
...
- **Decision A: API vs fine-tune vs build** — work the use case through the build-vs-buy decision paths below and compare 3-year TCO for each path.
- **Decision B: Risk classification under EU AI Act + US state laws** — classify the use case against the tier table below and list the required controls.
- **Decision C: API vs self-hosted economics** — model the workload's token volume against API pricing and self-hosted GPU costs to find the breakeven point.
...
To reach a use-case-specific recommendation, characterize the use case (volume, latency budget, accuracy bar, team size, budget), then compare each path's 3-year TCO and capability ceiling. See `references/model_buildvsbuy_strategy.md` for the full decision tree.
...
To classify a use case, map the decisions it affects, its users, geography, and sector against the tier table above and the state laws below, then derive the required-controls list for its tier.
...
**Typical breakeven (frontier-quality):** 100M–500M tokens/month, depending on model size and acceptable quality tradeoff. Below this, API wins. Above this, model the specific workload.

To find the breakeven for a specific workload, model tokens/day against current API per-token pricing versus GPU rental cost at realistic utilization, then test sensitivity to GPU rates and model size.
...
1. Define the use case: volume, latency, accuracy, team size, budget
2. Work it through the build-vs-buy paths above; compare 3-year TCO and breakeven for each path
3. Cross-check the budget commitment with the CFO
4. Cross-check engineering capacity with the CTO (especially for fine-tune)
5. Record the decision; treat multi-year vendor commitments as decisions to revisit on a fixed review date
...
1. Define the use case: decisions affected, users, geography, sector
2. Classify it against the EU AI Act tier table and US state patchwork above; derive the required-controls list
3. For HIGH-RISK: budget conformity assessment + registration
4. For LIMITED-RISK: implement transparency requirements
5. Cross-check contractual implications with legal counsel
6. Cross-check technical safeguards with the security lead
...
1. Characterize the workload: tokens/day, model size, latency, quality tolerance
2. Model API cost vs self-hosted cost at realistic utilization; find the breakeven volume
3. Run sensitivity scenarios (low/mid/high GPU rates)
4. Estimate migration cost (engineering time + risk)
5. Cross-check the capex commitment with the CFO
6. Cross-check platform readiness with the CTO
7. Record the decision; treat a signed GPU commitment as a decision to revisit on a fixed review date
...
4. Cross-check comp + leveling with the people lead
...
**The Evidence:** [numbers from the analysis, not adjectives]
